    ...
    S: AsyncRead + AsyncWrite + Unpin,
{
    ...
    req.parse(&buf)
        .map_err(|e| std::io::Error::new(std::io::ErrorKind::InvalidData, format!("httpup: {}", e)))?;
    ...
    if let Some(want) = &spec.path {
        ...
        if normalize(actual) != normalize(want) && !normalize(actual).starts_with(&format!("{}/", normalize(want).trim_end_matches('/'))) {
...
fn normalize(p: &str) -> String {
    ...
    if p.is_empty() || p == "/" { "/".into() } else { p.trim_end_matches('/').to_string() }
}
